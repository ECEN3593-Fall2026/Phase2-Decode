...
Note, for those using Icarus Verilog Version less than 12.0, the casting to the opcodes_t type causes problems for you (so you want to not have an explicit cast).  Doing that does not work for Version 12 (which is what we use), so it doesn't compile for us.  To solve, we put a conditional based on a compile time flag (VERNOT12).  If you have version less than 12, compile as follows: 
```
iverilog -DVERNOT12 -g2012 -o system_tb.out -c file_list.txt
```
...
* SystemVerilog code that builds without errors - 4 points (see note)
* SystemVerilog runs with some output - 1 point (see note)
